...
import {displayDate} from "@/utils/index.js";
...
const books = computed(() => booksStore.getSelectorItems)
...
const openEditDialog = (item) => {
  ...
  formData.value.bookIds = item.booksWithAuthors.map(book => Number(book.id))
...
const prepareCreateData = () => {
  ...
  if (!requestData.bookIds || !requestData.bookIds.length) delete requestData.bookIds;
  if (!requestData.client) delete requestData.client;
  return requestData;
}
...
<template>
  <div class="items-container">
    ...
    <v-card class="items-card" elevation="4">
      ...
      <div class="table-container">
        ...
        >
          ...
          <template v-slot:item.booksWithAuthors="{ item }">
            <template v-for="(book) in item.booksWithAuthors">
              {{ book.title }} <br/>
            </template>
          </template>
          ...
          <template v-slot:item.updated="{ item }">
            {{ item.updated ? displayDate(item.updated) : '-' }}
          </template>
    ...
    <scroll-center-dialog
      ...
      :title="createDialogOpened ? 'Добавить новый заказ' : 'Редактировать заказ'"
    >
      ...
      <div class="form-content mt-4">
        <v-text-field
          ...
          :error-messages="getError('client')"
          density="comfortable"
          clearable
        >
          <template v-slot:label>
            <span class="field-label">Информация о заказчике<span class="required-star">*</span></span>
          </template>
        </v-text-field>
        ...
        <v-select
          ...
          bg-color="grey-lighten-4"
          class="form-field"
          :error="hasError('bookIds')"
          :error-messages="getError('bookIds')"
          item-value="id"
          item-title="titleWithAuthor"
          density="comfortable"
          clearable
        >
          <template v-slot:label>
            <span class="field-label">Книги<span class="required-star">*</span></span>
          </template>
        </v-select>
      </div>
    </scroll-center-dialog>
